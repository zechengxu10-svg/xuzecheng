...
import { FruitImage } from './FruitImage.tsx';
...
export const FruitCard: React.FC<FruitCardProps> = ({ 
  ...
  return (
    <motion.button 
      ...
      onClick={onClick}
      className={`relative inline-flex flex-col w-96 rounded-[32px] shadow-nm-flat ${fruit.color} backdrop-blur-xl bg-white/40 border border-white/40 overflow-hidden group/card cursor-pointer transition-all duration-300 fruit-card text-left ${(isActive || isHovered) ? 'ring-4 ring-nm-accent ring-offset-4 ring-offset-nm-bg active-card' : ''}`}
    >
      <div className="relative h-56 overflow-hidden bg-nm-ink/5">
        ...
        <motion.div
          ...
          className={`absolute top-4 right-4 w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 z-20 ${
            isFavorite 
